...
clean <- function(data) {
  ...
  data_tbl$key <- seq.int(nrow(data_tbl))
  
  # Create a clean version where we'll store our cleaned variables
  clean_tbl <- data_tbl %>% select(key)
  ...
  device_decoded <- paste('[', paste(data_tbl$device, collapse = ','), ']') %>% fromJSON(flatten = T)
  ...
  # geoNetwork
  geo_decoded <- paste('[', paste(data_tbl$geoNetwork, collapse = ','), ']') %>% fromJSON(flatten = T)
  ...
  data_tbl %>% group_by(as.character(fullVisitorId)) %>% summarize(count = n()) %>% arrange(desc(count))
  
  # Social engagement type is not useful, only one value
  
  # totals
  totals_decoded <- paste('[', paste(data_tbl$totals, collapse = ','), ']') %>% fromJSON(flatten = T)
  ...
  # Traffic Source
  traffic_decoded <- paste('[', paste(data_tbl$trafficSource, collapse = ','), ']') %>% fromJSON(flatten = T)
  ...
  # Number of visits
  temp <- data_tbl %>% group_by(visitId) %>% summarize(num_visits = n())
  temp2 <- data_tbl %>% select(key, visitId)
  temp2 <- temp2 %>% left_join(temp) %>% select(-visitId)
  clean_tbl <- clean_tbl %>% left_join(temp2)
  rm(temp, temp2)
  
  # Finally join in the full visitor ID
  temp <- data_tbl %>% select(key, fullVisitorId)
  clean_tbl <- clean_tbl %>% left_join(temp)
  rm(temp)
